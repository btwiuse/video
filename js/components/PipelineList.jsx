              ...
              {!hasAnyPreview ? (
                ...
                <div className="mt-4 grid w-full max-w-full grid-cols-4 overflow-hidden rounded-xl border border-ink-700 bg-ink-700">
                  {PROJECT_PREVIEW_LABELS.map((label, index) => {
                    ...
                    return <div key={label} title={images.length ? `${label} · ${assetCount} 个素材` : label} className="flex min-w-0 flex-col border-r border-ink-700 last:border-r-0 bg-ink-800">
                      {images.length ? <div className="grid aspect-square min-h-0 w-full overflow-hidden bg-ink-700" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${rows}, minmax(0, 1fr))` }}>{cells.map((imageName, cellIndex) => imageName ? <img key={imageName} loading="lazy" src={artifactUrl(p.pipeline_id, imageName)} alt={`${p.name || '项目'} · ${label}`} className="block h-full min-h-0 w-full min-w-0 object-cover object-center"/> : <span key={`empty-${cellIndex}`} className="block min-h-0 bg-ink-800"/>)}</div> : <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-1 text-stone-500"><span className="text-sm leading-none">＋</span><span className="text-[11px] font-medium">{label}</span></div>}
                      {images.length > 0 && <div className="flex h-7 shrink-0 items-center justify-between border-t border-ink-600 bg-ink-700/70 px-2 text-[11px] font-semibold leading-none text-stone-200"><span>{label}</span><span className="text-brass-600">{assetCount}</span></div>}
                    </div>;
                  })}
                </div>
              )}
